...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- tr:flat-list-to-tree( element()*, xs:integer, xs:integer, xs:QName, xs:string, xs:string )
       ...
       which provide their list level as attribute value.
       
       an example can be found in test.xsl
  -->
  ...
  <xsl:function name="tr:flat-list-to-tree" as="element()*">
    ...
    <xsl:element name="{$wrapper-qname}" namespace="{namespace-uri-from-QName($wrapper-qname)}" exclude-result-prefixes="#all">
      <xsl:for-each-group select="$list" 
                          group-adjacent="boolean(self::*[xs:integer(replace(@*[name() eq $level-att-name],
                                                                     $level-att-regex,
                                                                     '')) = $level])">
        <xsl:choose>

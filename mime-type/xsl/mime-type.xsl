...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:function name="tr:fileext-to-mime-type" as="xs:string">
    ...
    <xsl:variable name="media-type" select="if( contains($file-extension, 'html')) then 'application/xhtml+xml' 
      ...
      else if($file-extension eq 'otf') then 'application/x-font-opentype'
      else if($file-extension eq 'eot') then 'application/vnd.ms-fontobject'
      else if($file-extension eq 'ttf') then 'application/x-font-ttf'
...
<!--    <xsl:variable name="media-type" select="lower-case(replace($fileref, '.*\.(.*)$', '$1'))"/>-->
    <xsl:variable name="file-extension" select="if( $media-type eq 'application/xhtml+xml') then 'html'
      ...
      else if($media-type eq 'application/vnd.ms-opentype') then 'otf'
      else if($media-type eq 'application/x-font-opentype') then 'otf'
      else if($media-type eq 'application/vnd.ms-fontobject') then 'eot'
      else if($media-type eq 'application/x-font-truetype') then 'ttf'
      else if($media-type eq 'application/x-font-ttf') then 'ttf'
      else if($media-type eq 'application/font-woff') then 'woff' 
